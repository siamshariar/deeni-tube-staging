...
const menuItems = [
  { icon: BookOpen, label: "Quran Translations", description: "Recitations and translations", href: "#" },
      ...
      <div className="flex">
        ...
        <div className="flex-1 md:pl-[240px] pt-[56px] md:pt-[80px] pb-nav-safe md:pb-6">
          ...
          <div className="max-w-[600px] mx-auto px-4 md:px-6">
            ...
            {isLoading ? (
              <div className="divide-y">{Array.from({ length: 5 }).map((_, i) => <div key={i} className="flex items-center gap-4 px-1 py-4"><Skeleton className="h-10 w-10 rounded-full" /><div className="flex-1 space-y-2"><Skeleton className="h-4 w-32" /><Skeleton className="h-3 w-48" /></div><Skeleton className="h-4 w-4" /></div>)}</div>
            ) : (
              <div className="divide-y">
                {menuItems.map((item) => (
                  <Link key={item.label} href={item.href} className="flex items-center gap-4 px-1 py-4 hover:bg-muted/50 transition-colors rounded-lg group">
                    ...
                    <div className="flex-1 min-w-0"><p className="text-sm font-medium">{item.label}</p>{item.description && <p className="text-xs text-muted-foreground">{item.description}</p>}</div>
            ...
            <div className="py-8 text-center"><p className="text-xs text-muted-foreground">Powered By - Deeni Info Tech</p></div>
          </div>
        </div>
      </div>
      <MobileNav />
