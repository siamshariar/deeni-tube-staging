"use client"

import { useState, useEffect, useCallback, useRef } from "react"
import { useParams, useRouter } from "next/navigation"
import Image from "next/image"
import Link from "next/link"
import {
  ArrowLeft,
  Bell,
  BellOff,
  BellRing,
  ChevronDown,
  MoreVertical,
  Search,
  Share,
  UserMinus,
  Globe,
} from "lucide-react"

import { Button } from "@/components/ui/button"
import { Tabs, TabsList, TabsTrigger } from "@/components/ui/tabs"
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar"
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from "@/components/ui/dropdown-menu"
import { Skeleton } from "@/components/ui/skeleton"

import AppHeader from "@/components/app-header"
import VideoCard from "@/components/video-card"
import DesktopSidebar from "@/components/desktop-sidebar"
import { useMediaQuery } from "@/hooks/use-media-query"
import { cn } from "@/lib/utils"
import MobileNav from "@/components/mobile-nav"

const channelData = {
  id: "islamic-guidance",
  name: "Islamic Guidance",
  handle: "@islamicguidance",
  avatar: "/medical-professional-profile.png",
  coverImage: "/vibrant-health-cover.png",
  subscribers: "2.5M subscribers",
  videos: "3.2K videos",
  description:
    "Islamic Guidance is dedicated to spreading authentic Islamic knowledge through high quality videos. Our channel features lectures from renowned scholars, Quran recitations, and educational content about Islam. We strive to provide accurate and beneficial content for Muslims and non-Muslims alike. Join us on this journey of learning and discovery.",
  website: "islamicguidance.com",
  facebook: "facebook.com/islamicguidance",
  twitter: "twitter.com/islamicguidance",
  youtube: "youtube.com/@islamicguidance",
  verified: true,
}

const videos = [
  { id: "v1", title: "The Purpose of Life - Powerful Islamic Reminder", thumbnail: "/diverse-group-exercising.png", views: "208K views", timeAgo: "2 hours ago", duration: "18:28" },
  { id: "v2", title: "Tafsir of Surah Al-Fatiha - Complete Explanation", thumbnail: "/vibrant-health-drink.png", views: "150K views", timeAgo: "2 days ago", duration: "25:15" },
  { id: "v3", title: "How to Pray Salah - Step by Step Guide", thumbnail: "/healthy-heart-motion.png", views: "500K views", timeAgo: "5 days ago", duration: "12:40" },
  { id: "v4", title: "Stories of the Prophets - Prophet Musa (AS)", thumbnail: "/healthy-pancreas-infographic.png", views: "1.2M views", timeAgo: "1 week ago", duration: "45:00" },
  { id: "v5", title: "The Day of Judgment - Signs and Events", thumbnail: "/sunny-health-lesson.png", views: "3.4M views", timeAgo: "2 weeks ago", duration: "32:10" },
  { id: "v6", title: "10 Tips for a Better Ramadan", thumbnail: "/food-avoidance-icons.png", views: "800K views", timeAgo: "3 weeks ago", duration: "15:30" },
]

const playlists = [
  { id: "p1", title: "Quran Tafsir Series", videoCount: 24, updatedAt: "Updated 2 weeks ago" },
  { id: "p2", title: "Stories of the Prophets", videoCount: 18, updatedAt: "Updated 1 month ago" },
  { id: "p3", title: "Islamic Lectures", videoCount: 32, updatedAt: "Updated 3 days ago" },
  { id: "p4", title: "Ramadan Special", videoCount: 15, updatedAt: "Updated 2 months ago" },
  { id: "p5", title: "Dawah Training", videoCount: 47, updatedAt: "Updated 1 week ago" },
  { id: "p6", title: "Short Reminders", videoCount: 12, updatedAt: "Updated 3 weeks ago" },
]

export default function ChannelPage() {
  const params = useParams()
  const router = useRouter()
  const isMobile = useMediaQuery("(max-width: 768px)")
  const [activeTab, setActiveTab] = useState("videos")
  const [showSearch, setShowSearch] = useState(false)
  const [searchQuery, setSearchQuery] = useState("")
  const [showFullDescription, setShowFullDescription] = useState(false)
  const [showAllLinks, setShowAllLinks] = useState(false)
  const [isSubscribed, setIsSubscribed] = useState(true)
  const [notificationType, setNotificationType] = useState("personalized")
  const [scrollY, setScrollY] = useState(0)
  const [showChannelNameInHeader, setShowChannelNameInHeader] = useState(false)

  const [loadingVideos, setLoadingVideos] = useState(true)
  const [videosList, setVideosList] = useState<typeof videos>([])
  const [hasMore, setHasMore] = useState(true)
  const [page, setPage] = useState(1)
  const observer = useRef<IntersectionObserver | null>(null)
  const loadingRef = useRef<HTMLDivElement>(null)

  const fetchVideos = useCallback(async (pageNum: number) => {
    await new Promise((resolve) => setTimeout(resolve, 1500))
    const newVideos = videos.map((video) => ({
      ...video,
      id: `${video.id}-p${pageNum}-${Math.random().toString(36).substring(4)}`,
      title: pageNum === 1 ? video.title : `${video.title} (part ${pageNum})`,
      views: `${Math.floor(Math.random() * 900 + 100)}K views`,
      timeAgo: `${Math.floor(Math.random() * 12) + 1} ${pageNum === 1 ? "days" : "weeks"} ago`,
    }))
    if (pageNum >= 3) setHasMore(false)
    return newVideos
  }, [])

  useEffect(() => { fetchVideos(1).then(setVideosList).finally(() => setLoadingVideos(false)) }, [fetchVideos])

  useEffect(() => {
    if (loadingVideos) return
    observer.current = new IntersectionObserver(([target]) => { if (target.isIntersecting && hasMore) setPage(p => p + 1) }, { rootMargin: "0px 0px 200px 0px" })
    if (loadingRef.current) observer.current.observe(loadingRef.current)
    return () => observer.current?.disconnect()
  }, [loadingVideos, hasMore])

  useEffect(() => {
    if (page === 1) return
    setLoadingVideos(true)
    fetchVideos(page).then((newV) => { setVideosList(p => [...p, ...newV]); setLoadingVideos(false) })
  }, [page, fetchVideos])

  useEffect(() => {
    const handleScroll = () => { setScrollY(window.scrollY); setShowChannelNameInHeader(window.scrollY > 200) }
    window.addEventListener("scroll", handleScroll)
    return () => window.removeEventListener("scroll", handleScroll)
  }, [])

  const channel = channelData

  const getNotificationIcon = (type: string) => {
    switch (type) {
      case "all": return <Bell className="h-5 w-5" />
      case "personalized": return <BellRing className="h-5 w-5" />
      case "none": return <BellOff className="h-5 w-5" />
      default: return <Bell className="h-5 w-5" />
    }
  }

  const getButtonBellIcon = () => {
    switch (notificationType) {
      case "all": return <Bell className="h-4 w-4 mr-2" />
      case "personalized": return <BellRing className="h-4 w-4 mr-2" />
      case "none": return <BellOff className="h-4 w-4 mr-2" />
      default: return <Bell className="h-4 w-4 mr-2" />
    }
  }

  return (
    <div className="min-h-screen bg-background">
      <AppHeader />

      {/* Mobile Back + Title */}
      {/* <div className="md:hidden flex items-center gap-3 px-4 py-2 pt-[56px] border-b bg-background">
        <button
          onClick={() => router.back()}
          className="flex items-center justify-center h-9 w-9 rounded-full hover:bg-muted transition-colors flex-shrink-0"
        >
          <ArrowLeft className="h-5 w-5" />
        </button>
        {showChannelNameInHeader && (
          <h1 className="text-base font-medium truncate">{channel.name}</h1>
        )}
      </div> */}

      <div className="flex">
        <DesktopSidebar className="hidden md:block" />

        <main className={cn("flex-1", isMobile ? "pb-nav-safe" : "md:pl-[240px] md:pt-[80px]")}>
          {/* Channel Banner */}
          <div className="relative w-full">
            <div className="w-full aspect-[3/1] md:aspect-[6/1.5] relative">
              <Image src={channel.coverImage} alt={`${channel.name} banner`} fill className="object-cover" priority />
            </div>
          </div>

          {/* Channel Info */}
          <div className="px-4 py-4 border-b">
            <div className="flex items-start gap-4">
              <Avatar className={cn("flex-shrink-0", isMobile ? "h-16 w-16" : "h-20 w-20")}>
                <AvatarImage src={channel.avatar} alt={channel.name} />
                <AvatarFallback>{channel.name.charAt(0)}</AvatarFallback>
              </Avatar>

              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1">
                  <h1 className="text-xl md:text-2xl font-bold truncate">{channel.name}</h1>
                  {channel.verified && (
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 text-gray-500 flex-shrink-0">
                      <path fillRule="evenodd" d="M8.603 3.799A4.49 4.49 0 0112 2.25c1.357 0 2.573.6 3.397 1.549a4.49 4.49 0 013.498 1.307 4.491 4.491 0 011.307 3.497A4.49 4.49 0 0121.75 12a4.49 4.49 0 01-1.549 3.397 4.491 4.491 0 01-1.307 3.497 4.491 4.491 0 01-3.497 1.307A4.49 4.49 0 0112 21.75a4.49 4.49 0 01-3.397-1.549 4.49 4.49 0 01-3.498-1.306 4.491 4.491 0 01-1.307-3.498A4.49 4.49 0 012.25 12c0-1.357.6-2.573 1.549-3.397a4.49 4.49 0 011.307-3.497 4.49 4.49 0 013.497-1.307zm7.007 6.387a.75.75 0 10-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 00-1.06 1.06l2.25 2.25a.75.75 0 001.14-.094l3.75-5.25z" clipRule="evenodd" />
                    </svg>
                  )}
                </div>
                <p className="text-sm text-muted-foreground">{channel.handle}</p>
                <p className="text-sm text-muted-foreground">{channel.subscribers} • {channel.videos}</p>

                {/* Description */}
                <p className="text-sm text-muted-foreground mt-1">
                  {showFullDescription ? (
                    <>
                      {channel.description}
                      <button onClick={() => setShowFullDescription(false)} className="text-blue-600 ml-1 hover:underline font-medium">Show less</button>
                    </>
                  ) : (
                    <>
                      {channel.description.length > 120 ? channel.description.substring(0, 120) + "..." : channel.description}
                      {channel.description.length > 120 && (
                        <button onClick={() => setShowFullDescription(true)} className="text-blue-600 ml-1 hover:underline font-medium">...more</button>
                      )}
                    </>
                  )}
                </p>

                {/* Links */}
                <div className="mt-2 space-y-1">
                  <Link href={`https://${channel.website}`} className="text-sm text-blue-600 hover:underline flex items-center gap-1">
                    <Globe className="h-3 w-3" />{channel.website}
                  </Link>
                  {showAllLinks ? (
                    <>
                      <Link href={`https://${channel.facebook}`} className="text-sm text-blue-600 hover:underline block">Facebook</Link>
                      <Link href={`https://${channel.twitter}`} className="text-sm text-blue-600 hover:underline block">Twitter</Link>
                      <Link href={`https://${channel.youtube}`} className="text-sm text-blue-600 hover:underline block">YouTube</Link>
                      <button onClick={() => setShowAllLinks(false)} className="text-xs text-blue-600 hover:underline font-medium">Show less</button>
                    </>
                  ) : (
                    <button onClick={() => setShowAllLinks(true)} className="text-xs text-blue-600 hover:underline font-medium">and 3 more links</button>
                  )}
                </div>

                {/* Subscribe + Share Buttons */}
                <div className="flex items-center gap-2 mt-4">
                  {isSubscribed ? (
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button className="rounded-full bg-black text-white hover:bg-gray-800 h-9 text-sm px-4 flex-shrink-0">
                          {getButtonBellIcon()}Subscribed<ChevronDown className="h-4 w-4 ml-2" />
                        </Button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent align="start" sideOffset={4} className="w-[240px] p-0">
                        <DropdownMenuItem
                          className={`py-3 cursor-pointer flex items-center gap-3 px-4 ${notificationType === "all" ? "bg-muted" : ""}`}
                          onClick={() => setNotificationType("all")}
                        >
                          {getNotificationIcon("all")}
                          <span>All</span>
                        </DropdownMenuItem>
                        <DropdownMenuItem
                          className={`py-3 cursor-pointer flex items-center gap-3 px-4 ${notificationType === "personalized" ? "bg-muted" : ""}`}
                          onClick={() => setNotificationType("personalized")}
                        >
                          {getNotificationIcon("personalized")}
                          <span>Personalized</span>
                        </DropdownMenuItem>
                        <DropdownMenuItem
                          className={`py-3 cursor-pointer flex items-center gap-3 px-4 ${notificationType === "none" ? "bg-muted" : ""}`}
                          onClick={() => setNotificationType("none")}
                        >
                          {getNotificationIcon("none")}
                          <span>None</span>
                        </DropdownMenuItem>
                        <div className="border-t">
                          <DropdownMenuItem
                            className="py-3 cursor-pointer flex items-center gap-3 px-4 text-red-500"
                            onClick={() => { setIsSubscribed(false); setNotificationType("personalized") }}
                          >
                            <UserMinus className="h-5 w-5" />
                            <span>Unsubscribe</span>
                          </DropdownMenuItem>
                        </div>
                      </DropdownMenuContent>
                    </DropdownMenu>
                  ) : (
                    <Button className="rounded-full bg-black text-white hover:bg-gray-800 h-9 text-sm px-4 flex-shrink-0" onClick={() => setIsSubscribed(true)}>
                      Subscribe
                    </Button>
                  )}
                  <Button variant="outline" className="rounded-full h-9 flex-shrink-0">
                    <Share className="h-4 w-4 mr-2" />Share
                  </Button>
                </div>
              </div>
            </div>
          </div>

          {/* Tabs */}
          <div className={`border-b ${isMobile ? "sticky top-[56px] bg-background z-20" : ""}`}>
            <Tabs defaultValue="videos" className="w-full" onValueChange={setActiveTab}>
              <TabsList className="h-12 bg-background w-full justify-start overflow-x-auto scrollbar-hide px-4">
                <TabsTrigger value="videos" className="px-4 py-1 data-[state=active]:border-b-2 data-[state=active]:border-black data-[state=active]:rounded-none data-[state=active]:bg-transparent data-[state=active]:shadow-none text-sm">Videos</TabsTrigger>
                <TabsTrigger value="shorts" className="px-4 py-1 data-[state=active]:border-b-2 data-[state=active]:border-black data-[state=active]:rounded-none data-[state=active]:bg-transparent data-[state=active]:shadow-none text-sm">Shorts</TabsTrigger>
                <TabsTrigger value="playlists" className="px-4 py-1 data-[state=active]:border-b-2 data-[state=active]:border-black data-[state=active]:rounded-none data-[state=active]:bg-transparent data-[state=active]:shadow-none text-sm">Playlists</TabsTrigger>
                <TabsTrigger value="posts" className="px-4 py-1 data-[state=active]:border-b-2 data-[state=active]:border-black data-[state=active]:rounded-none data-[state=active]:bg-transparent data-[state=active]:shadow-none text-sm">Posts</TabsTrigger>
              </TabsList>
            </Tabs>
          </div>

          {/* Search field */}
          {showSearch && (
            <div className="px-4 py-2 border-b">
              <input type="text" placeholder="Search channel" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full h-10 py-2 px-4 rounded-full border focus:outline-none focus:border-blue-500 text-sm" />
            </div>
          )}

          {/* Video Filters */}
          {activeTab === "videos" && (
            <div className="px-4 py-3 flex gap-2 overflow-x-auto scrollbar-hide">
              <Button variant="outline" className="rounded-full bg-black text-white hover:bg-gray-800 h-8 text-xs px-3">Latest</Button>
              <Button variant="outline" className="rounded-full h-8 text-xs px-3">Popular</Button>
              <Button variant="outline" className="rounded-full h-8 text-xs px-3">Oldest</Button>
            </div>
          )}

          {/* Videos Tab */}
          {activeTab === "videos" && (
            <>
              <div className="hidden md:grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 p-4">
                {videosList.map((video) => <VideoCard key={video.id} isHorizontal={false} />)}
                {loadingVideos && Array.from({ length: 4 }).map((_, i) => (
                  <div key={`sk-${i}`} className="flex flex-col">
                    <div className="relative aspect-video w-full"><Skeleton className="h-full w-full rounded-lg" /></div>
                    <div className="flex mt-2 gap-2"><Skeleton className="h-9 w-9 rounded-full flex-shrink-0" /><div className="flex-1 space-y-2"><Skeleton className="h-4 w-full" /><Skeleton className="h-3 w-3/4" /></div></div>
                  </div>
                ))}
                {hasMore && <div ref={loadingRef} className="col-span-full flex justify-center p-4" />}
              </div>
              <div className="flex flex-col md:hidden">
                {videosList.map((video) => <VideoCard key={video.id} isHorizontal={true} />)}
                {loadingVideos && Array.from({ length: 3 }).map((_, i) => (
                  <div key={`skm-${i}`} className="flex gap-3 p-3">
                    <Skeleton className="w-40 h-24 rounded-lg flex-shrink-0" />
                    <div className="flex-1 space-y-2"><Skeleton className="h-4 w-full" /><Skeleton className="h-3 w-3/4" /></div>
                  </div>
                ))}
                {hasMore && <div ref={loadingRef} className="flex justify-center p-4" />}
              </div>
            </>
          )}

          {/* Shorts Tab */}
          {activeTab === "shorts" && (
            <div className={cn("grid gap-4 p-4", isMobile ? "grid-cols-2 gap-2 p-2" : "grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6")}>
              {videosList.map((video) => (
                <div key={`sh-${video.id}`} className="flex flex-col cursor-pointer">
                  <div className="relative aspect-[9/16] w-full">
                    <Image src={`/placeholder-7zeab.png?height=480&width=270&query=shorts`} alt={video.title} fill className="object-cover rounded-lg" />
                    <div className="absolute bottom-2 right-2 bg-black bg-opacity-80 text-white text-xs px-1 py-0.5 rounded">{video.duration}</div>
                  </div>
                  <h3 className="font-medium text-sm line-clamp-2 mt-2">{video.title}</h3>
                  <p className="text-muted-foreground text-xs mt-0.5">{video.views}</p>
                </div>
              ))}
            </div>
          )}

          {/* Playlists Tab */}
          {activeTab === "playlists" && (
            <>
              <div className="hidden md:grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 p-4">
                {playlists.map((pl) => (
                  <Link href={`/playlist/${pl.id}`} key={pl.id} className="border rounded-lg p-4 hover:bg-gray-50 transition-colors">
                    <h3 className="font-medium text-base line-clamp-2">{pl.title}</h3>
                    <p className="text-muted-foreground text-xs mt-1">{pl.updatedAt}</p>
                    <div className="mt-4 flex items-center gap-1 text-sm text-muted-foreground">
                      <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 14.5v-9l6 4.5-6 4.5z"/></svg>
                      {pl.videoCount} videos
                    </div>
                  </Link>
                ))}
              </div>
              <div className="md:hidden">
                {playlists.map((pl) => (
                  <Link href={`/playlist/${pl.id}`} key={pl.id} className="block p-4 border-b hover:bg-gray-50">
                    <h3 className="font-medium text-base line-clamp-2">{pl.title}</h3>
                    <p className="text-muted-foreground text-xs mt-1">{pl.updatedAt}</p>
                    <p className="text-muted-foreground text-xs mt-1">{pl.videoCount} videos</p>
                  </Link>
                ))}
              </div>
            </>
          )}

          {/* Posts Tab */}
          {activeTab === "posts" && (
            <div className="p-8 text-center text-muted-foreground">No posts to display</div>
          )}

          <MobileNav />
        </main>
      </div>
    </div>
  )
}