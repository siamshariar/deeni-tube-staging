...
// ============= CONSTANTS (unchanged, no extra data) =============
...
export default function YouPage() {
  ...
  // --- user data from localStorage (set during sign-in) ---
  ...
  // --- load data on mount ---
  useEffect(() => {
    // Simulate loading (keep skeleton)
    const timer = setTimeout(() => setIsLoading(false), 600)
    return () => clearTimeout(timer)
  }, [])

  useEffect(() => {
    // Load user data from sign-in
    ...
    if (savedPrefs) {
      try {
        ...
        setHasSeenPopup(true) // already set preferences before
      } catch (e) {}
    }
  ...
  // --- save preferences to localStorage & show toast ---
  ...
  // --- toggle preferred language (multi-select) ---
  ...
  // --- first-time popup logic: only show if no preferences exist ---
  useEffect(() => {
    const hasPrefs = localStorage.getItem("youPagePreferences") !== null
    if (!hasPrefs && !hasSeenPopup) {
      // Show popup after a short delay (so user sees the page)
  ...
  const closeFirstTimePopup = useCallback(() => {
    ...
    savePreferences() // save whatever they selected
  }, [savePreferences])
  ...
  // --- Drawer content for mobile preferred languages ---
  const languageDrawerContent = (
    <div className="p-4 space-y-2 pb-8">
      <p className="text-sm text-muted-foreground mb-3">
        Select your preferred languages to personalise your content
      </p>
      ...
      <Dialog open={!localStorage.getItem("youPagePreferences") && hasSeenPopup} onOpenChange={(open) => !open && closeFirstTimePopup()}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Select your preferred languages to personalise your content feed
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="border rounded-lg divide-y max-h-96 overflow-y-auto">
              {/* Show ALL preferred languages, not just first 6 */}
              {preferredLanguageOptions.map((lang) => {
                const isSelected = preferredLanguages.includes(lang.code)
                return (
                  <button
            ...
            <Button className="w-full" onClick={closeFirstTimePopup}>
              Continue
            </Button>
          </div>
        </DialogContent>
      ...
      <Dialog open={showSignOutDialog} onOpenChange={setShowSignOutDialog}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to sign out? You will need to sign in again to access your preferences.
            </DialogDescription>
          </DialogHeader>
          <div className="flex gap-3 mt-2">
            <Button variant="outline" onClick={() => setShowSignOutDialog(false)} className="flex-1">
              Cancel
            </Button>
              ...
              <div className="flex items-center gap-4 p-4 rounded-xl bg-card border shadow-sm">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <button
                    ...
                    className="mt-1.5 flex items-center gap-1.5 text-sm text-destructive hover:text-destructive/80 transition-colors focus:outline-none focus:ring-2 focus:ring-destructive rounded-full px-2 py-0.5"
                    aria-label="Sign out"
                  >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 divide-x divide-y">
                {accountSections.map((section, idx) => (
                  <Link
                    key={idx}
                    href={section.href}
                    className="flex flex-col items-center justify-center gap-2 p-4 hover:bg-muted/50 transition-colors text-center group focus:outline-none focus:ring-2 focus:ring-primary focus:z-10"
                  >
                  ...
                  {isMobile ? (
                    <Drawer open={languageDrawerOpen} onOpenChange={setLanguageDrawerOpen}>
                      <DrawerTrigger asChild>
                        <button className="w-full flex items-center justify-between py-2 hover:bg-muted/50 rounded-lg px-2 transition-colors text-left">
                          <div>
                ...
                <div className="flex items-center gap-3">
                  {(["system", "light", "dark"] as ThemeMode[]).map((mode) => (
                    <button
                      ...
                      className={cn(
                        "flex-1 flex flex-col items-center gap-2 p-3 rounded-lg border-2 transition-all focus:outline-none focus:ring-2 focus:ring-primary",
                        theme === mode
